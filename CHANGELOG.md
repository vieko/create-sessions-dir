...
### Changed
- **Linear integration**: Replaced `linearis` CLI with Linear MCP
  - Removed `Bash(linearis:*)` from allowed-tools and settings.json
  - Updated `/start-session` to use available Linear MCP tools
  - Updated setup instructions to reference Linear MCP configuration

### Removed
- `linearis` CLI dependency (Linear MCP is the recommended integration)

## [0.3.7] - 2024-12-10

### Fixed
- **Monorepo path resolution**: Slash commands now work when launched from nested directories
  - All commands find `.sessions/` at git root using `git rev-parse --show-toplevel`
  - Fixed `/start-session` failing with "file not found" from monorepo subdirectories
  - Updated `/end-session`, `/archive-session`, `/document`, and `/plan` commands
